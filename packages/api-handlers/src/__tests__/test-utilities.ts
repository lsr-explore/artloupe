...
vi.mock("@artloupe/logger", () => ({
	logger: mockLogger,
	withLogging: (handler: (request: NextRequest) => Promise<Response>) => handler,
}));

// Helper function to create a proper mock NextRequest with headers
export const createMockRequest = (url: string, headers?: Record<string, string>, method: string = 'GET'): NextRequest => {
...
export const createMockPostRequest = (body: Record<string, unknown>, headers?: Record<string, string>): NextRequest => {
	const mockRequest = createMockRequest("http://localhost/api/test", headers, 'POST');
...
export const createMockGetRequest = (url: string, headers?: Record<string, string>): NextRequest => {
