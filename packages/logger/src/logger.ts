...
class LoggerImpl {
	...
	constructor(config: LoggerConfig = {}) {
		...
		if (pretty && process.env.ENABLE_PRETTY_LOGS === 'true' && globalThis.window === undefined && !process.env.NEXT_RUNTIME) {
			try {
				...
				console.warn('Pretty printing failed, falling back to basic logging:', error);
			}
		} 
	...
	logWithContext(level: 'info' | 'warn' | 'error' | 'debug', message: string, data?: unknown): void {
		const logData = {
			...this.context,
